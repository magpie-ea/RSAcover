...
```{r}
data <- read.csv("../../data/pilot-1b-n10/results.csv")
```
...
```{r}
# Define the positions and significance levels
sig_df <- data.frame(
  group1 = c("part_-match", "part_random", "sum_-match", "sum_+match"),
  group2 = c("part_+match", "part_+match", "sum_random", "sum_random"),
  y.position = c(105, 110, 105, 110),  # adjust based on your data
  significance = c("***", "***", "***", "***")
)

my_comparisons <- list( c("0.5", "1"), c("1", "2"), c("0.5", "2") )

data$condition <- paste(data$F1_NPforms, data$F2_matchness, sep = "_")
...
p1 <- ggplot(data, aes(x = condition, y = acceptability)) +
  geom_boxplot(aes(fill = F2_matchness)) +
  labs(x = "NP Structures", y = "Acceptability") +
...
# Show mean of false filler
data_filler %>% group_by(F1_NPforms) %>% summarise(mean(acceptability))
```
...
```
```{r}
# Compare sum random with 50
data_sum_random <- data %>% filter(F1_NPforms == "sum" & F2_matchness == "random")
data_part_random <- data %>% filter(F1_NPforms == "part" & F2_matchness == "random")
data_part_Nmatch <- data %>% filter(F1_NPforms == "part" & F2_matchness == "-match")

t.test(data_sum_random$acceptability, mu = 50, alternative = "greater")
t.test(data_part_random$acceptability, mu = 50, alternative = "greater")
t.test(data_part_Nmatch$acceptability, mu = 50, alternative = "greater")
